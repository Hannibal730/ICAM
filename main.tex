...
For model configuration, ICAM uses a lightweight MBConv-based visual encoder with an input size of $224 \times 224$ \citep{sandler2018mobilenetv2}.
The encoder feature map is pooled into a $7 \times 7$ grid, yielding $N{=}49$ encoder tokens of dimension 24.
The decoder uses one learnable query ($N_q{=}1$), six cross-attention layers, two attention heads, embedding dimension 24, dropout 0.1, and stochastic depth ratio 0.2.
As shown in Table~\ref{tab:param_breakdown}, the model has only 47,585 trainable parameters in total.

For training, we follow the Sewer-ML preprocessing protocol and train ICAM and all baselines from scratch for 90 epochs, consistent with the training schedule of Sewer-ML \citep{haurum2021sewer}.
We optimize cross-entropy loss with label smoothing 0.1 using AdamW (initial learning rate $1 \times 10^{-3}$, weight decay $1 \times 10^{-4}$). We use cosine annealing scheduling with $T_{\max}{=}90$ and $\eta_{\min}{=}1 \times 10^{-4}$.
To account for differences in dataset scale, we use a batch size of 256 for Sewer-ML and 16 for the private dataset.
Training is performed on a single NVIDIA RTX 5090 GPU.
...
\begin{table}[t]
    ...
    \resizebox{0.6\columnwidth}{!}{%
        \begin{tabular}{lr}
            ...
            \textbf{Encoder} & \textbf{15,840} \\
            \quad Convolution Layer & 15,792 \\
            \quad LayerNorm & 48 \\
            \addlinespace 
            
            \textbf{Decoder} & \textbf{31,392} \\
...
To reflect a practical deployment pipeline, we convert trained PyTorch checkpoints to ONNX format (opset version 17) and evaluate them with ONNX Runtime (v1.23.2).
All experiments are performed on a \textbf{Raspberry Pi 5} (4GB RAM), which features a Broadcom BCM2712 SoC with a quad-core ARM Cortex-A76 CPU and LPDDR4X memory.
This platform provides a realistic testbed under computational and power constraints widely encountered in industrial edge computing and robotics, in contrast to proxy efficiency metrics collected on server-class GPUs \citep{mathe2024comprehensive}.



For reproducibility on an embedded platform, latency and memory experiments are executed under system-level controlled settings.
...
CPU affinity is pinned to cores 0--3 to reduce scheduler-induced jitter and cache effects arising from core migration.
ONNX Runtime is configured with \texttt{intra\_op\_num\_threads}=4, \texttt{inter\_op\_num\_threads}=1, and \texttt{ORT\_SEQUENTIAL} execution mode, which keeps the operator-level parallelism fixed across runs and reduces scheduler-induced variance.
To prevent thread oversubscription and uncontrolled nested parallelism in external numerical libraries, we set the environment variables \texttt{OMP\_NUM\_THREADS}, \texttt{OPENBLAS\_NUM\_THREADS}, \texttt{MKL\_NUM\_THREADS}, \texttt{NUMEXPR\_NUM\_THREADS}, and \texttt{VECLIB\_MAXIMUM\_THREADS} to 1 for all latency and memory experiments.
The CPU governor is set to \texttt{performance} during benchmarking.
...
We measure inference latency and memory usage with a fixed protocol: warm-up (50 runs), timed phase (1{,}000 runs), random input, single-sample batch size ($B=1$), and seed 42. For robustness, each benchmark is repeated five times as independent fresh-process executions, and we report Mean $\pm$ Std across repeats.
For \textbf{latency}, we exclude the warm-up runs to eliminate initialization effects such as JIT compilation, cache warming, and dynamic frequency scaling, following the evaluation protocols established by the MLPerf Inference benchmark \citep{reddi2020mlperf}.
We measure the elapsed time using \texttt{time.perf\_counter\_ns()}. To capture both average performance and runtime stability (tail latency), we report the mean, P50, P90, P95, and P99 percentiles.
For \textbf{memory usage}, we report the Peak Resident Set Size (RSS) to evaluate the actual physical memory footprint during inference.
Captured via the standard GNU time utility (\texttt{/usr/bin/time -v}), this metric represents the maximum physical memory allocated by the process.
This includes persistent memory for weights and the non-persistent arena for activation maps, consistent with memory management architectures for TinyML systems \citep{david2021tensorflow}.
We additionally report internal RSS diagnostics by polling \texttt{proc.memory\_info().rss} at 1~ms intervals.
This high-frequency monitoring isolates memory overheads during model-load from inference phases, which is essential for assessing system multitenancy and operational limits on edge devices \citep{david2021tensorflow}.
...
We report Top-1 accuracy over all classes to ensure a comprehensive evaluation of global performance.
We also primary focus on the metrics for the \textbf{abnormal class}: precision, recall, and F1-score.
...
While this necessitates high recall, relying solely on recall is insufficient since requent false alarms (low precision) can reduce system reliability and increase manual verification costs \citep{chandola2009anomaly}.
Therefore, we report precision and F1-score alongside recall to provide a balanced assessment of anomaly detection capability.
...
We also use percentile calibration at the 99.99, which sets clipping thresholds by discarding extreme activation values beyond this percentile, mitigating the impact of outliers on the quantization scale while preserving resolution for the bulk of the activation distribution \citep{li2019fully}.
...
In this section, we validate the practical feasibility of ICAM by comparing it with baselines on a Raspberry Pi 5 under standardized resource constraints.



\begin{table}[b]
    \centering
    \caption{Comparison of model complexity in terms of FLOPs, parameters, and storage size. FLOPs are reported assuming $2 \times$ FLOPs per MAC. Size denotes the physical storage requirements of the converted ONNX file across different precisions.}
    \label{tab:unpruned_structure}
    \resizebox{0.9\columnwidth}{!}{%
        \begin{tabular}{l c r r r r}
            \toprule
            \multirow{2}{*}[-2.5pt]{\textbf{Model}} & 
            \multirow{2}{*}[-2.5pt]{\textbf{FLOPs (G)}} & 
            \multirow{2}{*}[-2.5pt]{\textbf{Params}} & 
            \multicolumn{3}{c}{\textbf{Size (MB)}} \\
            \cmidrule(lr){4-6}
             & & & \textbf{FP32} & \textbf{FP16} & \textbf{INT8} \\
            \midrule
            MobileNetV4-S \citep{qin2024mobilenetv4}    & 0.3853 & 2,495,586 & 9.49 & 4.76 & 2.65 \\
            Xie et al. \citep{xie2019automatic}         & 2.8696 & 9,160,194 & 34.95 & 17.4 & 8.77 \\
            DeiT-Tiny \citep{touvron2021training}       & 2.1493 & 5,524,802 & 21.18 & 10.6 & 5.81 \\
            MobileViT-XXS \citep{mehta2021mobilevit}    & 0.5384 &   951,666 & 3.80 & 2.05 & 1.62 \\
            ICAM (Ours)                                 & \textbf{0.1816} & \textbf{47,585} & \textbf{0.30} & \textbf{0.23} & \textbf{0.35} \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
\begin{table*}[h!]
\centering
\caption{Performance comparison across different precisions (FP32, FP16, INT8). Models are evaluated on Sewer-ML and Private dataset. Note that Precision, Recall and F1-score are reported with respect to the \textbf{Abnormal} class.}
\label{tab:unpruned_performance}
\resizebox{\textwidth}{!}{
...
% --- FP32 Block ---
\multirow{5}{*}{FP32}
 & MobileNetV4-S \citep{qin2024mobilenetv4} & 90.53 & 89.49 & 90.56 & 90.02 & 84.66 & 81.44 & \textbf{86.62} & 83.95 \\
 ...
 & MobileViT-XXS \citep{mehta2021mobilevit} & 91.16 & \textbf{90.72} & 90.53 & 90.62 & 82.60 & 81.41 & 80.89 & 81.15 \\
 & ICAM (Ours)                              & \textbf{91.32} & 89.60 & \textbf{92.31} & \textbf{90.94} & \textbf{86.43} & \textbf{87.25} & 82.80 & \textbf{84.97} \\
\midrule
% --- FP16 Block ---
\multirow{5}{*}{FP16} 
 & MobileNetV4-S \citep{qin2024mobilenetv4} & 90.53 & 89.50 & 90.55 & 90.02 & 84.66 & 81.44 & \textbf{86.62} & 83.95 \\
 ...
 & MobileViT-XXS \citep{mehta2021mobilevit} & 91.16 & \textbf{90.72} & 90.54 & 90.63 & 82.01 & 81.17 & 79.62 & 80.39 \\
 & ICAM (Ours)                              & \textbf{91.32} & 89.64 & \textbf{92.28} & \textbf{90.94} & \textbf{86.43} & \textbf{87.25} & 82.80 & \textbf{84.97} \\
\midrule
% --- INT8 Block ---
\multirow{5}{*}{INT8} 
 & MobileNetV4-S \citep{qin2024mobilenetv4} & 83.20 & \textbf{96.55} & 66.79 & 78.96 & 84.96 & 81.93 & \textbf{86.62} & 84.21 \\
 & Xie et al. \citep{xie2019automatic}     & 89.47 & 88.92 & 88.74 & 88.83 & 81.42 & 77.65 & 84.08 & 80.73 \\
 & DeiT-Tiny \citep{touvron2021training}   & 85.82 & 81.52 & 90.44 & 85.75 & 78.76 & 72.73 & \textbf{86.62} & 79.09 \\
 & MobileViT-XXS \citep{mehta2021mobilevit} & 90.41 & 91.85 & 87.42 & 89.58 & 82.89 & 82.35 & 80.25 & 81.29 \\
 & ICAM (Ours)                              & \textbf{91.26} & 89.70 & \textbf{92.05} & \textbf{90.86} & \textbf{86.43} & \textbf{87.25} & 82.80 & \textbf{84.97} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\begin{table*}[h!]
    \centering
    \caption{Inference latency and Peak Resident Set Size (RSS) performance analysis across precisions. 
    A warm-up phase was executed to stabilize the runtime state prior to measurement.
    Latency reports the average inference execution time, excluding model loading and warm-up overheads.
    Peak RSS captures the maximum physical memory footprint of the entire process lifetime, including interpreter startup, model loading, warm-up, and inference.}
    \label{tab:unpruned_latency_mem}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{ll c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c}
        \toprule
        \multirow{2}{*}[-2.5pt]{\textbf{Precision}} & \multirow{2}{*}[-2.5pt]{\textbf{Model}} & \multicolumn{8}{c}{\textbf{Latency (ms)}} & \multicolumn{2}{c}{\textbf{RSS (MB)}} \\
        ...
        % --- FP32 Block ---
        \multirow{5}{*}{FP32}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & 7.73 & 0.06 & 7.68 & 0.05 & 7.91 & 0.10 & 9.14 & 0.52 & 74.85 & 0.02 \\
         & Xie et al. \citep{xie2019automatic}        & 33.89 & 0.19 & 33.75 & 0.18 & 35.00 & 0.38 & 36.20 & 0.28 & 123.14 & 0.26 \\
         & DeiT-Tiny \citep{touvron2021training}      & 33.05 & 0.15 & 32.80 & 0.12 & 34.37 & 0.27 & 36.11 & 0.41 & 91.70 & 0.90 \\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 22.22 & 0.17 & 22.09 & 0.17 & 22.82 & 0.22 & 24.57 & 0.28 & 78.34 & 0.06 \\
         & ICAM (Ours)                                & \textbf{7.66} & \textbf{0.04} & \textbf{7.61} & \textbf{0.05} & \textbf{7.86} & \textbf{0.06} & \textbf{9.04} & \textbf{0.38} & \textbf{71.67} & \textbf{0.27} \\
        \midrule
        % --- FP16 Block ---
        \multirow{5}{*}{FP16}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & 4.44 & 0.01 & 4.42 & 0.02 & 4.54 & 0.09 & 5.04 & 0.26 & 72.23 & 0.01 \\
         & Xie et al. \citep{xie2019automatic}        & 25.98 & 0.11 & 25.91 & 0.11 & 26.51 & 0.15 & 27.35 & 0.13 & 183.42 & 4.23 \\
         & DeiT-Tiny \citep{touvron2021training}      & 53.11 & 0.17 & 52.88 & 0.15 & 54.61 & 0.26 & 55.67 & 0.32 & 115.28 & 1.80\\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 37.07 & 0.19 & 36.90 & 0.16 & 38.03 & 0.22 & 39.15 & 0.47 & 81.53 & 0.12 \\
         & ICAM (Ours)                                & \textbf{3.84} & \textbf{0.02} & \textbf{3.81} & \textbf{0.02} & \textbf{3.97} & \textbf{0.11} & \textbf{4.67} & \textbf{0.46} & \textbf{67.38} & \textbf{0.24} \\
        \midrule
        % --- INT8 Block ---
        \multirow{5}{*}{INT8}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & \textbf{2.68} & \textbf{0.01} & \textbf{2.66} & \textbf{0.02} & \textbf{2.78} & \textbf{0.10} & \textbf{3.30} & \textbf{0.30} & \textbf{67.28} & \textbf{0.26} \\
         & Xie et al. \citep{xie2019automatic}        & 16.10 & 0.07 & 16.05 & 0.05 & 16.37 & 0.10 & 17.11 & 0.18 & 109.07 & 0.16 \\
         & DeiT-Tiny \citep{touvron2021training}      & 37.12 & 0.10 & 36.92 & 0.08 & 38.46 & 0.30 & 39.84 & 0.95 & 81.59 & 0.29 \\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 17.00 & 0.05 & 16.92 & 0.05 & 17.32 & 0.06 & 18.49 & 0.41 & 77.56 & 0.28 \\
         & ICAM (Ours)                                & 5.02 & 0.01 & 4.99 & 0.01 & 5.13 & 0.03 & 5.76 & 0.24 & 72.50 & 0.21 \\
        \bottomrule
\end{tabular}
}
\end{table*}
...
\begin{table*}[h!]
    \centering
    \caption{Comprehensive memory usage analysis across different precisions. 
            Values represent RSS deltas ($\Delta$) in MB measured via \texttt{psutil} sampling at 1 ms intervals.
            Load $\Delta$ measures the memory increase during initialization.
            Inference $\Delta$ is measured relative to the baseline before warm-up (cold-start), and after warm-up (steady-state).
            Inference $\Delta$ is measured relative to the post-load baseline.
            Total $\Delta$ represents the peak total memory footprint increase from the pre-load state.}
    \label{tab:unpruned_mem_delta}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{ll c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c c@{ $\pm$ }c}
        \toprule
        \multirow{2}{*}[-2.5pt]{\textbf{Precision}} & \multirow{2}{*}[-2.5pt]{\textbf{Model}} & \multicolumn{6}{c}{\textbf{Cold Start (MB)}} & \multicolumn{6}{c}{\textbf{Steady State (MB)}} \\
        \cmidrule(lr){3-8} \cmidrule(lr){9-14}
         & & \multicolumn{2}{c}{\textbf{Load $\Delta$}} & \multicolumn{2}{c}{\textbf{Infer. $\Delta$}} & \multicolumn{2}{c}{\textbf{Total $\Delta$}} & \multicolumn{2}{c}{\textbf{Load $\Delta$}} & \multicolumn{2}{c}{\textbf{Infer. $\Delta$}} & \multicolumn{2}{c}{\textbf{Total $\Delta$}} \\
        \midrule
        % --- FP32 Block ---
        \multirow{5}{*}{FP32}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & 15.30 & 0.00 & \textbf{0.45} & \textbf{0.01} & 15.75 & 0.01 & 15.30 & 0.00 & \textbf{0.03} & \textbf{0.01} & 15.75 & 0.02 \\
         & Xie et al. \citep{xie2019automatic}        & 38.28 & 0.00 & 28.22 & 0.01 & 66.50 & 0.01 & 38.28 & 0.00 & \textbf{0.03} & \textbf{0.01} & 66.51 & 0.01 \\
         & DeiT-Tiny \citep{touvron2021training}      & 25.79 & 1.16 & 2.31 & 0.02 & 28.11 & 1.15 & 25.30 & 1.13 & \textbf{0.03} & \textbf{0.01} & 27.61 & 1.13 \\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 6.92 & 0.45 & 11.92 & 0.52 & 18.84 & 0.13 & 6.95 & 0.48 & 0.04 & 0.01 & 18.83 & 0.14 \\
         & ICAM (Ours)                                & \textbf{2.40} & \textbf{0.01} & 12.03 & 0.02 & \textbf{14.43} & \textbf{0.01} & \textbf{2.40} & \textbf{0.01} & 0.04 & 0.01 & \textbf{14.43} & \textbf{0.01} \\
        \midrule
        % --- FP16 Block ---
        \multirow{5}{*}{FP16}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & 7.97 & 0.00 & 3.18 & 0.01 & 11.15 & 0.01 & 7.97 & 0.00 & 0.03 & 0.01 & 11.15 & 0.01 \\
         & Xie et al. \citep{xie2019automatic}        & 61.23 & 0.92 & 43.57 & 5.52 & 104.80 & 6.37 & 60.52 & 1.37 & 0.03 & 0.01 & 102.32 & 6.77 \\
         & DeiT-Tiny \citep{touvron2021training}      & 46.34 & 1.34 & \textbf{1.88} & \textbf{0.98} & 48.22 & 2.13 & 45.92 & 1.30 & 0.03 & 0.01 & 48.10 & 1.34 \\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 9.24 & 0.01 & 11.23 & 0.23 & 20.48 & 0.18 & 9.17 & 0.02 & \textbf{0.02} & \textbf{0.01} & 20.54 & 0.16 \\
         & ICAM (Ours)                                & \textbf{3.79} & \textbf{0.01} & 6.72 & 0.01 & \textbf{10.51} & \textbf{0.01} & \textbf{3.78} & \textbf{0.01} & 0.04 & 0.02 & \textbf{10.52} & \textbf{0.02} \\
        \midrule
        % --- INT8 Block ---
        \multirow{5}{*}{INT8}
         & MobileNetV4-S \citep{qin2024mobilenetv4}   & 7.28 & 0.05 & \textbf{0.96} & \textbf{0.01} & \textbf{8.24} & \textbf{0.05} & 7.30 & 0.03 & \textbf{0.02} & \textbf{0.01} & \textbf{8.25} & \textbf{0.03} \\
         & Xie et al. \citep{xie2019automatic}        & 28.27 & 0.01 & 30.08 & 0.01 & 58.35 & 0.01 & 28.28 & 0.00 & 0.03 & 0.01 & 58.34 & 0.00 \\
         & DeiT-Tiny \citep{touvron2021training}      & 11.89 & 0.24 & 5.88 & 0.02 & 17.78 & 0.24 & 11.59 & 0.22 & 0.03 & 0.01 & 17.49 & 0.22 \\
         & MobileViT-XXS \citep{mehta2021mobilevit}   & 8.59 & 0.03 & 8.48 & 0.01 & 17.07 & 0.03 & 8.60 & 0.04 & \textbf{0.02} & \textbf{0.01} & 17.07 & 0.04 \\
         & ICAM (Ours)                                & \textbf{4.47} & \textbf{0.01} & 10.16 & 0.01 & 14.63 & 0.02 & \textbf{4.48} & \textbf{0.02} & 0.03 & 0.01 & 14.64 & 0.02 \\
        \bottomrule
\end{tabular}
}
\end{table*}

\paragraph{Unpruned Performance}
Tables~\ref{tab:unpruned_structure} through \ref{tab:unpruned_mem_delta} summarize the experimental results for unpruned models. 
Table~\ref{tab:unpruned_structure} highlights the extreme compactness of ICAM, which requires orders of magnitude fewer parameters and FLOPs than all baselines. 
Notably, the INT8 file size for ICAM is slightly larger than its FP32 counterpart; this overhead stems from the QDQ representation, where auxiliary QuantizeLinear/DequantizeLinear nodes and per-channel metadata dominate storage costs at such a minute scale \citep{gholami2022survey}.

Table~\ref{tab:unpruned_performance} demonstrates that ICAM achieves the best accuracy and abnormal-class F1 on both datasets, as well as the highest recall on Sewer-ML, effectively minimizing missed defect detections.
Crucially, ICAM maintains this robust performance across FP32, FP16, and INT8 precisions. 
In contrast, while MobileNetV4-S attains high recall on the Private dataset, it suffers from lower precision and exhibits significant instability under quantization; specifically, its recall drops severely to 66.79\% in the INT8 regime.

Efficiency metrics in Table~\ref{tab:unpruned_latency_mem} show that ICAM offers the fastest inference and the lowest Peak RSS in FP32 and FP16. 
While MobileNetV4-S achieves lower latency in INT8, this comes at the cost of the aforementioned recall degradation. 
Interestingly, reduced precision does not uniformly yield efficiency gains for larger models; for instance, Xie et al. exhibits its highest Peak RSS under FP16, and DeiT-Tiny incurs higher latency in FP16 compared to FP32.

Finally, Table~\ref{tab:unpruned_mem_delta} shows that load memory deltas correlate with model size (ICAM being the smallest), while steady‑state inference deltas are negligible across models, indicating effective memory reuse after warm‑up.
For ICAM, cold‑start inference deltas exceed load deltas, suggesting that activation buffers dominate transient memory footprints even when weights are minimal.”




\paragraph{Iso-FLOPs Performance}
Under the iso-FLOPs constraint ($\approx$0.18\,GFLOPs), Tables~\ref{tab:sewer_ml_iso_flops} and \ref{tab:private_iso_flops} compare performance on Sewer-ML and the Private dataset, isolating representational efficiency at a fixed arithmetic budget. The corresponding iso-FLOPs latency/Peak RSS and RSS-delta results quantify whether matched FLOPs translate into matched runtime and memory on Raspberry Pi 5; any gaps reflect architectural differences in operator mix and memory access.





\paragraph{Iso-Params Performance}
Under the iso-Params constraint ($\approx$47.59K parameters), Tables~\ref{tab:sewer_ml_iso_params } and \ref{tab:private_iso_params} compare performance on both datasets, revealing which architectures remain reliable under a strict storage budget. The matching iso-Params efficiency tables provide latency/Peak RSS and RSS-delta comparisons, highlighting that parameter parity does not necessarily imply equal runtime or activation memory due to depth and feature-map footprints.
...
% Tables~\ref{tab:sewer_ml_iso_params } and \ref{tab:private_iso_params} present the comparison results where all baselines are heavily pruned to match the parameter count of ICAM ($\approx$ 31K parameters).
...
% Table~\ref{tab:sewer_ml_iso_params } shows the performance of baselines on the Sewer-ML dataset, which pruned to match the parameter level of ICAM.
